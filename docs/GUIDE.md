...
- **DET 管控的插件一并停用**：停用范围 = DET 管理范围内的全部插件 —— 全局插件库里当时真正开着的插件（常驻按 loader 实际状态判断，动态看会话实例）**＋** loader 里所有非框架（`@deepseek-ai/*` 除外）、非 DET 自身、且当前未禁用的常驻插件（即 DET「已安装插件」里能看到的那些，如 `topo`）。被停用的清单会显示在页面上（区分「常驻已安装 · 已卸载」/「常驻 · 已卸载 / 会话实例已停」），方便核对「确实都停了」；同时写一份快照 `det.master.paused`，**重新打开总开关时按快照自动恢复**（常驻重新装载 + 会话启用记录还原，客户端据此在对应会话重新启用）。本来就没开的插件不进快照，所以恢复时不会凭空打开。DET 自己与框架组件永不被动。
   > 注意：被 loader 卸载的常驻插件，其**前端界面**要等一次页面刷新（强刷）才会消失；后端能力（如 `topo_ds_ask`）立即失效。
