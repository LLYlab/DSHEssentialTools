...
- **DET 管控的插件一并停用**：全局插件库里当时真正开着的插件（`dbs` / `topo` 等）也会被停 —— 常驻插件经 loader 实时卸载，动态插件停掉各会话实例。被停用的清单会显示在页面上（「已随总开关停用的 DET 管控插件」），方便核对「确实都停了」；同时写一份快照 `det.master.paused`，**重新打开总开关时按快照自动恢复**（常驻重新装载 + 会话启用记录还原，客户端据此在对应会话重新启用）。本来就没开的插件不进快照，所以恢复时不会凭空打开。
