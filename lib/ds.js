...
/** 余额缓存 TTL(毫秒):前端每 5 秒刷新,TTL 略低于 5s 以确保每次轮询拿到新值。 */
const BALANCE_TTL = 4 * 1000;
/** 价格缓存 TTL(毫秒):定价变动极少,6 小时一刷。 */
const PRICE_TTL = 6 * 60 * 60 * 1000;
/** 单次请求超时(毫秒):无超时会让余额轮询/定价刷新挂死。 */
...
export async function fetchDsBalance(apiKey, opts) {
  ...
  try {
    resp = await fetch(BALANCE_URL, {
      ...
      redirect: "follow",
      // 跨源重定向时 fetch 会剥离 Authorization(CORS 非通配请求头),故 key 不会被转发到第三方;
      // 这里再加超时,避免端点不响应时挂死。最终主机仍在下方校验。
      signal: (opts && opts.signal) || AbortSignal.timeout(DS_TIMEOUT_MS),
    });
  } catch (e) {
