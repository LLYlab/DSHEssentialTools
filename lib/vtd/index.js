...
export class VtdStore {
  ...
  // ── 树边的自有存储（DSH ≥0.1.2 上不再写会话日志）─────────────────────
  //
  // 背景：DET 原本把树边作为 `conversation/link` 事件追加进会话日志。
  // DSH 0.1.2-alpha.4 起，插件自定义事件类型必须带 `ignorable: true`，
  // 否则读路径会拒绝重建整个会话；而 v0→v1 迁移器对未知历史事件类型
  // **无条件拒绝**（"refuses unknown historical events even when ignorable"），
  // 且 `Session.append(type, data, ...opts)` 只读 `sourceEventSeqs`/`surfaceOp`，
  // **根本没有传 `ignorable` 的入口**。
  //
  // 结论：在新宿主上继续写 `conversation/link` 会让父会话变成读不回来的会话。
  // 故改为存进 DET 自己的存储域；读时再合成等价事件，使 deriveTree 与全部
  // 读取方零改动。旧宿主（≤0.1.1-rc.2）行为不变，仍写会话日志。

  /** 树边的 settings 键前缀。 */
  static edgeKey(sessionId) { return "det.branchEdges." + String(sessionId || ""); }

  /**
   * 读取某会话的自有树边。
   * @returns {Promise<Array<object>>} 边数组（无则空表）。
   */
  async loadEdges(sessionId) {
    if (!sessionId) return [];
    const res = await this.getSetting(VtdStore.edgeKey(sessionId));
    if (!res || !res.ok) return [];
    return Array.isArray(res.value) ? res.value : [];
  }

  /**
   * 追加/覆盖一条树边（按 branchId 去重，与 deriveTree 的语义一致）。
   * @returns {Promise<{ok:boolean, error?:string}>}
   */
  async appendEdge(sessionId, data) {
    if (!sessionId) return { ok: false, error: "缺少 sessionId" };
    if (!data || typeof data.branchId !== "string") return { ok: false, error: "树边缺少 branchId" };
    const edges = await this.loadEdges(sessionId);
    const next = edges.filter((e) => !e || e.branchId !== data.branchId);
    next.push(data);
    return await this.setSetting(VtdStore.edgeKey(sessionId), next);
  }

  /** 列出某会话的自有树边。 */
  async listEdges(sessionId) { return await this.loadEdges(sessionId); }
