...
const DOMAIN_SPEC = defineDomain({
  ...
  tables: {
    branches: domainTable(z.object({
      ...
      anchorSeq: z.number(),
      forkBoundary: z.number().optional(),
...
export class VtdStore {
  ...
  async branchListAll() {
    ...
    if (bad) return bad;
    const out = [];
    for (const [branchId, rec] of this.domain.table("branches").entries()) {
      out.push({
        branchId,
  ...
  async branchCreate(sessionId, messageId, label, resolveSeq) {
    ...
    // 父链：创建时的活动分支即父分支（祖先路径用于"节点子分支选择"计算）
    const parentBranchId = this.activeBySession.get(sessionId);
    const record = {
      branchId,
      sessionId,
      anchorSeq,
    ...
    if (parentBranchId !== undefined) record.parentBranchId = parentBranchId;
    await this.domain.table("branches").put(branchId, record);
    return { ok: true, branchId, anchorSeq, parentBranchId: parentBranchId || null };
  }

  /** 记录一条"真实子会话"分支(由 EssentialToolsService 先冷建子会话,再调此方法落域)。 */
  async recordChildBranch(opts) {
    const bad = await this._unavailable();
    if (bad) return bad;
    const sessionId = opts && opts.sessionId;
    const childSessionId = opts && opts.childSessionId;
    const anchorSeq = opts && opts.anchorSeq;
    if (typeof sessionId !== "string" || sessionId === "" || typeof childSessionId !== "string" || childSessionId === "" || !Number.isSafeInteger(anchorSeq) || anchorSeq < 0) {
      return { ok: false, error: "缺少会话/子会话 id 或锚点 seq" };
    }
    const branchId = mintBranchId();
    const parentBranchId = this.activeBySession.get(sessionId);
    const record = {
      ...
      sessionId,
      anchorSeq,
      forkBoundary: typeof opts.forkBoundary === "number" ? opts.forkBoundary : undefined,
      childSessionId,
      label: typeof opts.label === "string" ? opts.label.slice(0, 60) : "",
      ranges: [],
      createdAt: Date.now(),
      active: false,
    };
    if (parentBranchId !== undefined) record.parentBranchId = parentBranchId;
    await this.domain.table("branches").put(branchId, record);
    return { ok: true, branchId, childSessionId, anchorSeq, parentBranchId: parentBranchId || null };
  }
  ...
  async branchSwitch(sessionId, branchId) {
    ...
    const table = this.domain.table("branches");
    // "trunk"/"" = 切回主线：取消该会话所有活动分支
    if (branchId === "trunk" || branchId === "") {
      for (const [id, rec] of table.entries()) {
        if (rec.sessionId === sessionId && rec.active === true) {
    ...
    return { ok: true, branchId, childSessionId: target.childSessionId || null };
  }
