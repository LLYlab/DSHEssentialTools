...
/** 来源类型。 */
const ORIGIN_KINDS = ["cordis", "url", "github", "installed"];
...
const DOMAIN_SPEC = defineDomain({
  ...
  tables: {
    plugins: domainTable(z.object({
      ...
      // 常驻永久插件(已随 DSH boot 装载,如 dbs):enable 时不重复 spawn,只记录会话启用。
      permanent: z.boolean().default(false),
      moduleName: z.string().optional(),
      // 常驻插件全局二分开关:false = 禁用(loader.update disabled 实时卸载)。
      globallyEnabled: z.boolean().default(true),
    ...
    // 启动健康记录:记录「连续启动失败次数」,供启动安全策略(多次失败→全部禁用)使用。
    // key 固定为 "boot";consecutiveFail 跨重启持久化。
    boot: domainTable(z.object({
      key: z.string(),
      consecutiveFail: z.number().default(0),
      lastBootAt: z.number().optional(),
      lastHealthy: z.boolean().optional(),
    })),
  },
});
...
export class GlobalPluginStore {
  ...
  async _open() {
    // 防重入:并发首次访问(设置页 + 对话内工具同拍触发)只允许一次 `open`,
    // 否则宿主 `storageDomain.open` 会对同一域名抛出 `already-open`,把整个库标成不可用。
  ...
  async list() {
    ...
    if (un) return un;
    try {
      // 存储域表只暴露 get/entries/keys/size,没有 all():读取全部记录用 entries()。
      const rows = [...this.domain.table("plugins").entries()].map((entry) => entry[1]);
  ...
  async create(input) {
    ...
    // 常驻永久插件(带 moduleName/permanent)可无动态代码;其余必须 host/client 至少其一。
    const permanent = input.permanent === true || (typeof input.moduleName === "string" && input.moduleName !== "");
    if (!host && !client && !permanent) return { ok: false, error: "插件缺少代码(host/client 至少其一)" };
    ...
    const record = {
      ...
      ...typeof input.moduleName === "string" && input.moduleName !== "" ? { moduleName: input.moduleName } : {},
      globallyEnabled: input.globallyEnabled === false ? false : true,
  ...
  async updateMeta(id, patch) {
    ...
    if (typeof patch.globallyEnabled === "boolean") rec.globallyEnabled = patch.globallyEnabled;
  ...
  async upsert(input) {
    ...
    if (existing.ok) {
      ...
      // 没带新代码就不要动已有代码:否则「扫描已安装插件 → 纳入」(code 传空)
      // 会把库里这条记录原有的 host/client 动态代码抹掉。
      if (host !== undefined) rec.host = host;
      ...
      if (permanent) rec.permanent = true;
      if (typeof input.moduleName === "string" && input.moduleName !== "") rec.moduleName = input.moduleName;
  ...
  /** 读取启动健康记录(无则返回默认:consecutiveFail=0)。 */
  async getBootHealth() {
    const un = await this._unavailable();
    if (un) return { consecutiveFail: 0, lastBootAt: 0, lastHealthy: true };
    try {
      const rec = await this.domain.table("boot").get("boot");
      if (!rec) return { consecutiveFail: 0, lastBootAt: 0, lastHealthy: true };
      return {
        consecutiveFail: typeof rec.consecutiveFail === "number" ? rec.consecutiveFail : 0,
        lastBootAt: typeof rec.lastBootAt === "number" ? rec.lastBootAt : 0,
        lastHealthy: rec.lastHealthy !== false,
      };
    } catch (e) {
      return { consecutiveFail: 0, lastBootAt: 0, lastHealthy: true };
    }
  }

  /** 写入启动健康记录(覆盖整条)。 */
  async setBootHealth(health) {
    const un = await this._unavailable();
    if (un) return un;
    try {
      const rec = {
        key: "boot",
        consecutiveFail: Number.isFinite(health.consecutiveFail) && health.consecutiveFail > 0 ? Math.floor(health.consecutiveFail) : 0,
        lastBootAt: Date.now(),
        lastHealthy: health.lastHealthy !== false,
      };
      await this.domain.table("boot").put("boot", rec);
      return { ok: true, health: rec };
    } catch (e) {
      return { ok: false, error: String(e && e.message ? e.message : e) };
    }
  }
...
const SUSPICIOUS_PATTERNS = [
  ...
  { re: /\bchild_process\b/i, label: "child_process(进程执行)" },
  // \b 很重要:否则 "browserExec(" 也会命中(大小写不敏感),产生误报。
  { re: /require\s*\(\s*["'](node:)?child_process|execFile|\bexec\s*\(/i, label: "exec / execFile" },
