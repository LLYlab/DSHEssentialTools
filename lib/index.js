...
class EssentialToolsService extends TypertRemoteService {
  ...
  async msgRegenerate(args) {
    ...
    // 重答内容：当前（可能已被编辑替换）的问题内容 —— "编辑后没有变化于那个断点发送"
    ...
    return { ok: true, branchId: branchId, anchorSeq: q.seq, code: code };
  }

  // ── VTD：分支视图（主线/分支 + 消息流 + 节点子分支选择 + 替换折叠）───────
  ...
  async branchView(args) {
    ...
    // 无分支：主线视图 = 整个当前 surface（替换折叠；跳过 branch-reask 副本）
    if (branches.length === 0) {
      const msgs = collapseWindowEvents(events, () => true).map(viewMessage).filter((m) => !m.reask);
      views.trunk = { messages: msgs.slice(-500) };
      return { ok: true, branches: [], views: views, activeBranchId: null, trunk: true };
    }

    // 活动分支 + 祖先链（parentBranchId 上溯）
    const active = branches.find((b) => b.active) || null;
    const chain = new Map(); // branchId -> true（活动路径上的分支）
    let cur = active;
    while (cur) {
      chain.set(cur.branchId, true);
      cur = cur.parentBranchId ? branches.find((b) => b.branchId === cur.parentBranchId) : null;
    }
    // 每节点的子分支（anchorSeq 相同的分支，按 createdAt 排序 → 创建顺序 1..n）
    ...
    for (const branch of branches) {
      const ranges = branch.ranges || [];
      const inWindow = (s) => s <= branch.anchorSeq || ranges.some((r) => s >= r.from && s <= r.to);
      let msgs = collapseWindowEvents(events, inWindow).map(viewMessage).filter((m) => !m.reask);
      // 前缀过长时保留尾部（最近）
      const prefixCount = msgs.filter((m) => m.seq <= branch.anchorSeq).length;
      if (prefixCount > PREFIX_LIMIT) msgs = msgs.slice(prefixCount - PREFIX_LIMIT);
      // 逐消息标注：该节点子分支 + 活动路径在此节点的选择
      for (const m of msgs) {
        const children = childrenBySeq.get(m.seq) || [];
        m.childBranches = children.map((c, i) => ({ branchId: c.branchId, index: i + 1 }));
        const chosen = children.find((c) => chain.has(c.branchId));
        m.branchIndex = chosen ? (children.indexOf(chosen) + 1) : null;
        m.selector = children.length >= 2 && chosen !== undefined;
      }
      views[branch.branchId] = { messages: msgs };
    }
    return { ok: true, branches: branches, views: views, activeBranchId: active ? active.branchId : null };
  }
