...
/**
 * 容器/内置组件前缀(cordis:include 等)。
 *
 * 为什么必须单独认这一类:profile 的 cordis.yml 是由 `cordis:include` 这个条目**持有**的
 * 一棵子树 —— 那份「常驻已安装」清单里的 dbs/dlt/topo id 全是 `include:dbs` 这种孩子。
 * 把 include 当普通插件停用,等于去拆整棵 profile 插件树(顺带把刚停掉的插件按配置重建
 * 回来),所以停用清单里**绝不能**出现它。
 */
const CONTAINER_PREFIX = "cordis:";
...
class EssentialToolsService extends TypertRemoteService {
  ...
  async _disableAllGlobalPlugins(reason) {
    ...
    for (const p of res.plugins) {
      ...
      if (p.moduleName === DET_PLUGIN_NAME) continue;
      // 停掉所有会话中的实例。
      for (const sid of Object.keys(p.sessions || {})) {
        await this._stopSession(p, sid).catch(() => { /* ignore */ });
      }
      // 有 loader 条目就以「真卸载」为准(不看记录里 permanent 怎么写,记录可能过期);
      // 容器/内置(cordis:include)永不碰:它是一棵子树的持有者,停它会拆掉整棵 profile
      // 插件树 —— 连记录的全局开关都不要替它改(否则又留下「记录说禁了、实际还在跑」)。
      if (p.moduleName) {
        const found = this._loaderEntryByModule(p.moduleName);
        if (found && this._isContainerEntry(found.entry)) continue;
  ...
  /**
   * 该 loader 条目是不是「容器/内置」——代表的是**一棵子树**(`cordis:include` 持有整份
   * cordis.yml,dbs/dlt/topo 全都是它的孩子 `include:xxx`),绝不能当插件停用:
   * 停它 = 拆整棵 profile 插件树,而 include 那侧随后按配置重建孩子,
   * 刚停掉的插件又被拉起来 —— 这正是「关了 DET,下辖插件却没关」的根因。
   */
  _isContainerEntry(entry) {
    if (!entry) return true;
    if (entry.subtree) return true; // 拥有子树 = 容器
    const name = entry.options && typeof entry.options.name === "string" ? entry.options.name : "";
    return name.indexOf(CONTAINER_PREFIX) === 0;
  }

  /**
   * 条目此刻的**真实**状态(以 loader 为准):disabled 覆盖「自己 disabled」与
   * 「某个祖先 disabled」两种情形,所以它就是「这个插件到底还在不在跑」。
   * @returns {{found:boolean, disabled:boolean, phase:(string|null)}}
   */
  _entryRuntimeState(moduleName) {
    const found = moduleName ? this._loaderEntryByModule(moduleName) : null;
    if (!found) return { found: false, disabled: false, phase: null };
    const fiber = found.entry && found.entry.fiber;
    return {
      found: true,
      disabled: found.entry.disabled === true,
      phase: fiber ? (FIBER_PHASE[fiber.state] ?? null) : null,
    };
  }

  /**
   * 单个暂停条目「到底停没停」。
   * 不猜、不假设:库里的会话型插件看记录里还有没有会话映射;常驻插件看 loader 的真实状态。
   */
  async _pauseItemState(item) {
    // 绑定了 loader 条目 → 以 loader 的真实状态为准(记录里的 permanent 可能写错)。
    const st = this._entryRuntimeState(item.moduleName);
    if (st.found) {
      return st.disabled
        ? { applied: true, reason: "" }
        : { applied: false, reason: "loader 里仍在运行(" + (st.phase || "active") + ")" };
    }
    // 没绑定(纯动态插件)→ 看记录里还有没有会话实例(有 = 还开着)。
    const got = await this.global.get(item.id);
    if (!got.ok) return { applied: true, reason: "" }; // 记录都没了 → 没有实例在跑
    const left = Object.keys(got.plugin.sessions || {});
    return left.length === 0
      ? { applied: true, reason: "" }
      : { applied: false, reason: "仍有 " + left.length + " 个会话实例" };
  }

  /**
   * 执行后核对:真停了才算停;没停的补一次,仍没停就如实标 applied=false + reason。
   * 设置页据此显示「未能停用」,而不是照着快照假装都停了。
   */
  async _verifyPause(pause, loader) {
    const pending = [];
    for (const item of pause) {
      const state = await this._pauseItemState(item).catch(() => ({ applied: false, reason: "核对失败" }));
      item.applied = state.applied;
      item.reason = state.reason;
      if (!state.applied && item.moduleName) pending.push(item);
    }
    if (pending.length === 0) return pause;
    // 补一次:loader 收尾/容器重建都可能让第一次没落到实处。
    await new Promise((resolve) => setTimeout(resolve, 250));
    for (const item of pending) {
      const found = this._loaderEntryByModule(item.moduleName);
      if (found && !this._isContainerEntry(found.entry) && loader && typeof loader.update === "function") {
        try { await loader.update(found.entryId, { disabled: true }); } catch (e) { /* 下面按真实状态记 */ }
      }
      const again = await this._pauseItemState(item).catch(() => ({ applied: false, reason: "核对失败" }));
      item.applied = again.applied;
      item.reason = again.reason;
    }
    return pause;
  }
  ...
  // 执行完必须**核对**(见 _verifyPause),并把核对结果写进快照。

  /**
   * 总开关关闭:停用 **DET 管控范围内**的插件 —— 也就是「DET 全局插件库」里的插件。
   *
   * 界限必须清楚(DLT 的教训):不在库里的常驻插件(dlt 等)不属于 DET,它们有自己的
   * 开关,总开关**绝不碰**;所以这里不再盲扫 loader(上一版就是这么把 DLT 和
   * cordis:include 容器一起卷进去的)。
   *
   * 库里的一条记录要能被停,得**绑定到 loader 条目**(记录里有 moduleName,通常由
   * 「全局插件管理 → 扫描已安装插件 → 纳入」写进去)。没绑定、又声称自己是常驻开着的,
   * 就进 unmanaged 清单如实报给设置页,而不是靠猜名字去停别人的东西。
   */
  async _pauseManagedPlugins() {
    ...
    const unmanaged = [];
    for (const p of res.plugins) {
      if (p.moduleName === DET_PLUGIN_NAME) continue; // 绝不碰自己
      const entryInfo = p.moduleName ? this._loaderEntryByModule(p.moduleName) : null;
      // 容器/内置:不进停用清单(它由 loader 自己管,停它反而会拆树)。
      if (entryInfo && this._isContainerEntry(entryInfo.entry)) continue;
      const sessions = p.sessions || {};
      const sids = Object.keys(sessions);
      const entryOn = !!(entryInfo && entryInfo.entry.disabled !== true);
      // 记录说「我是常驻且开着」,但找不到对应 loader 条目 → DET 停不了它。
      // 如实记下来(提示去「纳入」),不进恢复快照(它本来就没被停过)。
      if (!entryInfo && p.permanent === true && p.globallyEnabled !== false) {
        unmanaged.push({
          id: p.id,
          name: p.name || "",
          moduleName: p.moduleName || "",
          reason: p.moduleName
            ? "loader 里找不到条目 " + p.moduleName
            : "库里这条记录没有绑定已安装条目(去「全局插件管理 → 扫描已安装插件 → 纳入」绑定后,总开关才能停它)",
        });
        continue;
      }
      // 本来就没开(无常驻装载、也无会话实例)→ 不进快照,避免「恢复」时凭空打开。
      if (sids.length === 0 && !entryOn) continue;
      pause.push({
        id: p.id,
        name: p.name || "",
        kind: "library",
    ...
    // 核对(含一次补刀),把**真实**结果写回快照;停不下来的如实记 applied=false。
    await this._verifyPause(pause, loader).catch(() => { /* 核对失败也不影响已经停掉的那部分 */ });
    if (pause.length > 0) await this.vtd.setSetting("det.master.paused", pause).catch(() => {});
    // DET 管不着的那些(DLT 之类不在库里的插件根本不会出现在这里;这里是「库里有记录但绑定不上」)。
    await this.vtd.setSetting("det.master.unmanaged", unmanaged).catch(() => {});
    return { ok: true, paused: pause, unmanaged };
  }
  ...
  async _resumeManagedPlugins() {
    ...
    for (const item of list) {
      ...
      // 旧快照里可能残留容器条目(旧版本误把 cordis:include 当插件停了):
      // 容器由 loader 自己管,不要替它开开关。
      if (item.moduleName && this._isContainerEntry(this._loaderEntryByModule(item.moduleName)?.entry)) {
      ...
      if (item.kind === "installed") {
        ...
        if (entry && loader && typeof loader.update === "function") {
          try {
            await loader.update(entry.entryId, { disabled: false });
      ...
      if (item.permanent === true && rec.permanent === true) {
        ...
        if (entry && !this._isContainerEntry(entry.entry) && loader && typeof loader.update === "function") {
   ...
   * 每条带 applied —— 是刚才**核对过**的真实结果(applied=false 表示当时没能停掉),
   * 这样设置页能显示「未能停用」而不是照着快照假装都停了。
   * 想看此刻的实时状态,用 gpScanInstalled(它以 loader 为唯一事实源)。
   */
  async gpMasterState(args) {
    ...
    const un = await this.vtd.getSetting("det.master.unmanaged");
    const unmanaged = un && un.ok && Array.isArray(un.value) ? un.value : [];
    const paused = list.map((p) => ({
      id: p.id,
      name: p.name || "",
      kind: p.kind || "library",
      permanent: p.permanent === true,
      ...
      // 旧快照没有这个字段:当成「未核对」而不是「失败」。
      applied: p.applied !== false,
      reason: p.applied === false ? (p.reason || "未能停用") : "",
      unverified: p.applied === undefined,
    }));
    return {
      ok: true,
      ...
      stoppedCount: paused.filter((p) => p.applied === true).length,
      failedCount: paused.filter((p) => p.applied === false).length,
      // 库里声称常驻、但绑定不上 loader 条目的记录:没被停,也不在恢复快照里。
      unmanaged,
