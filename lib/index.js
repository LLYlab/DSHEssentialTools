...
const METHOD_NAMES = [
  ...
  "gpStoreInspect", "gpStoreSummarize", "gpInstall", "gpSetLevel", "gpSetMeta",
...
function registerGlobalPluginTools(ctx, service) {
  ...
  if (sp && typeof sp.section === "function") {
    sp.section({
      ...
      order: 116,
      text: "# DET 全局插件\n\nDET 维护一个进程级「全局插件库」:每个插件有名称、描述与五个档位(全局启用 always / 对话AI可自行决定启用 ai-auto / 对话内AI需审批启用 ai-approve / 不再会有新启用 frozen / 全局禁用 disabled),你可以在对话内通过 det_global_plugin_list / det_global_plugin_enable / det_global_plugin_disable 按档位启用或停用。启用前先 list 查看档位;等待审批时(awaiting-approval)不要重试;被拒绝(frozen/disabled)后不要再请求。全局插件代码与动态 Cordis 插件一样在当前进程执行,拥有真实权限。",
    });
  }
}
