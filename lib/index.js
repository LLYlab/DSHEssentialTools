...
import { STORE_SOURCES, crawlStore, filterItems, installRepoOf } from "./store.js";
...
class EssentialToolsService extends TypertRemoteService {
  ...
  async detFeatureSet(args) {
    ...
    const keys = ["file", "run", "ver", "vtd", "mda", "plugins", "approve", "mms", "secCmdAudit", "secPromptDefense"];
    const clean = {};
    for (const k of keys) {
      if (typeof patch[k] === "boolean") clean[k] = patch[k];
    }
    if (Object.keys(clean).length === 0) return { ok: false, error: "无有效开关字段(仅接受 file/run/ver/vtd/mda/plugins/approve/mms/secCmdAudit/secPromptDefense 布尔值)" };
    ...
    if (clean.mda === false) { try { await this.mda.setMode("native"); } catch (e) { /* ignore */ } }
    // 2) 关闭「插件管理」开关 → 自动禁用所有全局插件(停各会话实例;常驻插件置为禁用并实时卸载)。
    if (clean.plugins === false) { try { await this._disableAllGlobalPlugins("det-feature-off"); } catch (e) { /* ignore */ } }
    // 3) MMS / 安全审计开关变化 → 立即重同步 DET 的动态工具与系统提示注入,无需刷新。
    try { await this._syncDetRuntimeFeatures(); } catch (e) { /* ignore */ }
  ...
  /** 是否允许某个 rank 的网络操作(当前档位 rank >= 需要最低 rank)。 */
  async _webAllows(minRank) {
    const lv = await this._webPerm();
    return lv.rank >= minRank;
  }
  ...
  /** 尽力判断会话是否 Full access 权限模式;无法确认时安全回退 false(需审批)。 */
  async _isFullAccess(sessionId) {
    try {
      const sessions = this.ctx.get("sessions");
      if (sessions && typeof sessionId === "string" && sessionId !== "") {
        const s = sessions.get(sessionId);
        if (s && s.projections && typeof s.projections.faceOf === "function") {
          const proj = s.projections.faceOf("permissions");
          if (proj && typeof proj.getSnapshot === "function") {
            const snap = proj.getSnapshot();
            if (snap && snap.currentValue === "danger-full-access") return true;
          }
        }
      }
    } catch (e) { /* 忽略 */ }
  ...
  /**
   * 执行一条浏览器命令(经本地扩展)。调用方(模型 det_browser 工具)必须已把高危项交审批。
  ...
  async _syncDetRuntimeFeatures() {
    ...
    if (f.secCmdAudit || f.secPromptDefense) this._registerSecPreExecute();
  ...
  /** 尽力创建目录(Windows cmd mkdir,失败静默;目录已存在也静默成功)。 */
  async ensureDir(dir) {
  ...
  async mdaCreateNoWorkspaceAgent(args) {
    ...
    const boundary = Math.max(0, (Array.isArray(liveParent.events) ? liveParent.events.length : 0) - 8);
  ...
  tools.register(defineTool({
    ...
    async execute(args) {
      ...
      return { source: res.source || "github", items: (res.items || []).map(function (it) {
        return { fullName: it.fullName, name: it.name, description: it.description, stars: it.stars, categories: it.categories, verificationStatus: it.verificationStatus, source: it.source };
      }) };
    },
  }));
...
/**
 * 注册浏览器控制模型工具 det_browser + 高危审批门禁。
 * 高危:web 权限第4档(使用用户浏览器)才允许;非 Full access 模式下执行浏览器动作需用户审批。
 * 审批通过 tools/pre-execute 瀑布 {kind:'ask'} 触发(产品审批服务;若缺失则降级为拒绝)。
 */
function registerBrowserTools(ctx, service) {
  ...
  tools.register(defineTool({
    name: "det_browser",
    description: "通过本地浏览器控制扩展,操作用户已登录的浏览器(需网络权限=使用用户浏览器;扩展开启;高危操作默认需审批)。action 可为 read_text/read_dom/screenshot/get_url/get_title/navigate/click/fill/run。tabId 可选(默认=当前活动标签页);maxLen 控制读取长度。只操作本机浏览器,不访问第三方站点。",
    parameters: {
      action: { type: "string", required: true, description: "read_text | read_dom | screenshot | get_url | get_title | navigate | click | fill | run" },
  ...
  // 高危审批门禁:det_browser 的浏览器动作在非 Full access 时走产品审批。
  try {
    ctx.on("tools/pre-execute", async (exec, next) => {
      try {
        if (!exec || exec.name !== "det_browser") return next();
        const sessionId = exec && exec.agent && exec.agent.id;
        const fullAccess = await service._isFullAccess(sessionId);
        if (fullAccess) return next();
        // 需要审批:ask。若审批服务缺失,注册表会把 ask 降级为拒绝。
        return { kind: "ask", reason: "高危浏览器操作:请确认允许 DET 操作你的浏览器(仅当权限为 Full access 时免审)。" };
      } catch (e) {
        return next();
      }
    }, { global: true, prepend: true });
    ...
    try {
      ...
      const body = lv.rank >= WEB_LEVELS[WEB_LEVELS.length - 1].rank
        ? "# DET 网络权限\n当前档位: " + lv.label + "。可自由发起网络访问。\n" +
            (lv.key === "browser"
              ? "浏览器控制:当网络权限为「使用用户浏览器」且用户已加载 DSH 浏览器扩展并将其开启时,可用 det_browser 操作当前浏览器(只读 read_text/read_dom/screenshot、写 navigate/click/fill/run)。高危:非 Full access 权限模式下执行需用户审批。扩展侧由用户在 关闭/只读/只写/启用 四档中选择,DSH 只读。\n"
              : "")
        : "# DET 网络权限\n当前档位: " + lv.label + "(" + lv.desc + ")。你发起的网络访问受此档位约束,低于该档位的操作(如通用网页抓取/下载/浏览器操作)会被 DET 拦截并返回错误。\n" +
            (lv.key === "api"
              ...
              : "") +
            "可选档位:\n" + text;
      if (disposer) { const d = disposer; disposer = null; d(); }
...
function apply(ctx, config) {
  ...
  registerBrowserTools(ctx, service);
  // 按持久化开关装载 MMS 工具/系统提示 + 安全审计监听;并在 apply 时注册网络权限提示。
  service._syncDetRuntimeFeatures().catch(() => { /* 尽力而为 */ });
  registerWebPermPrompt(ctx, service);
  return service;
}
