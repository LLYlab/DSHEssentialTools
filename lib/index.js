...
/** 沿 surface replace 链取某 seq 消息的"当前有效内容"（可能已被编辑替换）。 */
function currentMessageContentBySeq(session, seq, type) {
  ...
  for (const ev of events) {
    ...
    if (op && op.op === "replace" && op.start === curSeq) {
      curSeq = ev.seq;
      replaced = ev.data;
    }
  }
  if (replaced !== null) {
    return type === "user/message" ? replaced.content : (replaced.message && replaced.message.content);
  }
  const orig = events.find((e) => e.seq === seq);
  if (!orig || !orig.data) return null;
  return type === "user/message" ? orig.data.content : (orig.data.message && orig.data.message.content);
}

/**
 * 把消息 id 解析为"当前 surface 事件"（沿单消息 replace 链前进）。
 * 重复编辑/回退时，旧 id 指向的事件已被替换出 surface，必须用最新替换者。
 * 若最新事件被区间 replace（compaction）遮蔽，返回 {shadowed: true}。
 */
function resolveCurrentSurfaceEvent(session, messageId) {
  const events = Array.isArray(session && session.events) ? session.events : [];
  let ev = findMessageEvent(session, messageId);
  if (!ev) return null;
  let curSeq = ev.seq;
  let current = ev;
  for (const cand of events) {
    if (cand.seq <= curSeq) continue;
    const op = cand.surfaceOp;
    if (!op || op.op !== "replace") continue;
    if (op.start === curSeq && (cand.type === "user/message" || cand.type === "assistant/message")) {
      curSeq = cand.seq;
      current = cand;
    } else if (op.start <= curSeq && curSeq <= op.end && op.start !== curSeq) {
      // 被区间 replace（如 compaction）整体遮蔽
      return { shadowed: true, seq: curSeq };
    }
  }
  return current;
}
...
/**
 * 在窗口内折叠 surface 替换：被 replace（单条或区间）遮蔽的旧事件隐藏，
 * 保留最新替换者（各自带自己的 seq）。窗口外的替换不影响本窗口。
 * @param events - 全部事件。
 * @param isIn - (seq) => boolean，是否在分支窗口内。
 * @returns 窗口内可见的消息事件，按 seq 升序。
 */
function collapseWindowEvents(events, isIn) {
  const inWin = events.filter((ev) => isViewMessage(ev) && isIn(ev.seq));
  const bySeq = new Set(inWin.map((ev) => ev.seq));
  const hidden = new Set();
  for (const ev of inWin) {
    const op = ev.surfaceOp;
    if (!op || op.op !== "replace" || !Number.isInteger(op.start) || !Number.isInteger(op.end)) continue;
    for (let s = op.start; s <= op.end; s++) {
      if (bySeq.has(s)) hidden.add(s);
    }
  }
  return inWin.filter((ev) => !hidden.has(ev.seq)).sort((a, b) => a.seq - b.seq);
}
...
class EssentialToolsService extends TypertRemoteService {
  constructor(ctx, config) {
    ...
    // 分支子会话的 agent 句柄（branchId -> AgentHandle），用于续写/回答；插件卸载时统一释放。
  ...
  /** 分支边界：目标消息之前最近的轮次结束 seq；无闭合轮次则取首个轮次开始前的事件（或目标前一条）。 */
  forkBoundaryFor(session, targetSeq) {
    const events = Array.isArray(session && session.events) ? session.events : [];
    let lastEnd = -1;
    for (let i = 0; i < targetSeq; i++) {
      const ev = events[i];
      if (ev && ev.type === "turn/end") lastEnd = i;
    }
    if (lastEnd >= 0) return lastEnd;
    // 无任何闭合轮次：找第一个 turn/start
    let firstStart = -1;
    for (let i = 0; i < events.length; i++) {
      if (events[i] && events[i].type === "turn/start") { firstStart = i; break; }
    }
    if (firstStart < 0) return targetSeq - 1; // 全会话无轮次 → 目标前一条
    if (targetSeq <= firstStart) return targetSeq - 1; // 目标在首个轮次开始前
    return firstStart - 1; // 目标在首个开放轮次内 → 分叉点取首个轮次开始前的事件（无则 -1）
  }

  /**
   * 冷建分支子会话：persistence.create + append（种子 = 父事件 0..forkBoundary + 可选分歧事件），
   * 不进入活跃会话存储（后端真实对话 = 前端分支，前端不把它当活跃会话）。
   * 之后用 agentLoop.resume 驱动子会话回答。
   */
  async createChildBranch(parentSession, anchorSeq, divergence, label) {
    const persistence = this.ctx.get("sessionPersistence");
    if (!persistence) return { ok: false, error: "sessionPersistence 服务不可用" };
    const boundary = this.forkBoundaryFor(parentSession, anchorSeq);
    if (boundary < 0) return { ok: false, error: "该位置位于未闭合轮次内，无法分叉（请选择轮次结束后的消息）" };
    ...
    const now = Date.now();
    let seq = boundary + 1;
    const divergenceEvents = (divergence || []).map((d) => {
      const ev = { type: d.type, seq: seq++, time: now, data: d.data };
      if (d.surfaceOp) ev.surfaceOp = d.surfaceOp;
      return ev;
    });
    const meta = {
      version: 0,
    ...
    try {
      ...
      await persistence.append(childId, [...seed, ...divergenceEvents]);
    } catch (e) {
      return { ok: false, error: "创建分支会话失败: " + String(e && e.message ? e.message : e) };
    }
    const rec = await this.vtd.recordChildBranch({
      sessionId: parentSession.id,
      childSessionId: childId,
      anchorSeq,
      forkBoundary: boundary,
      label: label || "",
    });
    if (!rec.ok) return rec;
    return { ok: true, branchId: rec.branchId, childSessionId: childId, anchorSeq, forkBoundary: boundary };
  }

  /** 确保分支子会话的 agent 已运行（resume；已运行则复用）。 */
  async resumeChildAgent(childId) {
    const agentLoop = this.ctx.get("agentLoop");
    if (!agentLoop) return { ok: false, error: "agentLoop 服务不可用" };
    if (this.childAgents.has(childId)) return { ok: true };
    try {
  ...
  /** 读取子会话的事件（live 优先，否则冷读）。 */
  async childSessionEvents(childId) {
    ...
    try {
      ...
      return loaded && loaded.events ? loaded.events : null;
    } catch (e) {
      return null;
    }
  }
  ...
  /** 按 id 恢复程序版本（回退前自动备份；只动代码文件）。 */
  async _restoreVersionById(id) {
    const fs = this.fs();
  ...
  /** 回退代码到"断点时间之前最近的一个程序版本快照"（无快照则跳过）。 */
  async restoreCodeToBreakpoint(messageTime) {
    const list = await this.readManifest();
    let target = null;
    for (const v of list) {
      if ((v.time || 0) <= messageTime && (target === null || v.time > target.time)) target = v;
    }
    if (!target) return { ok: true, skipped: true, reason: "断点之前无程序版本快照" };
    return this._restoreVersionById(target.id);
  }

  async verProgRestore(args) {
    const id = args && args.id ? String(args.id) : "";
    if (id === "") return { ok: false, error: "缺少版本 id" };
    return this._restoreVersionById(id);
  }
  ...
  async treeList(args) {
    ...
    // 分支打包于对话下：每个会话节点附加其 VTD 分支（真实子会话 = 后端对话）
    const branchesBySession = new Map();
    const vtdList = await this.vtd.branchListAll();
    if (vtdList.ok) {
      for (const b of vtdList.branches) {
        const arr = branchesBySession.get(b.sessionId) || [];
        arr.push(b);
        branchesBySession.set(b.sessionId, arr);
      }
    }
    const attach = (node) => {
      const kids = (childrenOf[node.id] || []).slice().sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
      node.children = kids;
      const brs = (branchesBySession.get(node.id) || []).slice().sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
      if (brs.length > 0) node.branches = brs.map((b) => ({ branchId: b.branchId, childSessionId: b.childSessionId, anchorSeq: b.anchorSeq, label: b.label, active: b.active === true }));
  ...
  async msgEdit(args) {
    ...
    const resolved = resolveCurrentSurfaceEvent(session, messageId);
    if (!resolved) return { ok: false, error: "消息不存在" };
    if (resolved.shadowed) return { ok: false, error: "该消息已被压缩遮蔽，无法编辑" };
    const ev = resolved;
    ...
    try {
      // 在目标消息位置冷建真实子会话分支；编辑后的消息进入分支（后端新对话），前端仍内联展示。
      let divergence = null;
      ...
      const created = await this.createChildBranch(session, ev.seq, divergence, "编辑 @" + ev.seq);
      if (!created.ok) return created;
      if (needsAnswer) {
        const resumed = await this.resumeChildAgent(created.childSessionId);
        if (!resumed.ok) return { ok: false, error: resumed.error, branchId: created.branchId, childSessionId: created.childSessionId };
        const agents = this.ctx.get("agents");
        const childAgent = agents && agents.get(created.childSessionId);
        if (!childAgent || typeof childAgent.followup !== "function") {
          return { ok: false, error: "分支 agent 不可用", branchId: created.branchId, childSessionId: created.childSessionId };
        }
        const edited = createUserMessage({
          source: ev.data.source,
          content: contentFromText(newText),
        });
        childAgent.followup(edited);
      }
      await this.vtd.branchSwitch(sessionId, created.branchId);
      let code = null;
      if (args.rollbackCode === true) {
        code = await this.restoreCodeToBreakpoint(ev.time);
      }
      return { ok: true, branchId: created.branchId, childSessionId: created.childSessionId, anchorSeq: ev.seq, code: code };
    } catch (e) {
  ...
  async restoreContent(sessionId, messageId, targetContent, cause) {
    ...
    const resolved = resolveCurrentSurfaceEvent(session, messageId);
    if (!resolved) return { ok: false, error: "消息不存在" };
    if (resolved.shadowed) return { ok: false, error: "该消息已被压缩遮蔽，无法回退" };
    const ev = resolved;
    const check = this.editableBlocks(ev);
    if (!check.ok) return check;
  ...
  async msgRollback(args) {
    ...
    let code = null;
    if (args.rollbackCode === true) {
      const ev = findMessageEvent(this.ctx.get("sessions") && this.ctx.get("sessions").get(sessionId), messageId);
      code = await this.restoreCodeToBreakpoint(ev ? ev.time : 0);
    }
    return { ok: true, restoredVersion: target.versionIndex, target: toggle.target, code: code };
  }
  ...
  async msgRegenerate(args) {
    ...
    // 在问题处冷建真实子会话分支；把问题重新投入子会话 agent 重答（前端内联展示分支）。
    const created = await this.createChildBranch(session, q.seq, null, "重新生成 @" + ev.seq);
    if (!created.ok) return created;
    const resumed = await this.resumeChildAgent(created.childSessionId);
    if (!resumed.ok) return { ok: false, error: resumed.error, branchId: created.branchId, childSessionId: created.childSessionId };
    const currentContent = currentMessageContentBySeq(session, q.seq, "user/message");
    const reask = createUserMessage({
      source: { kind: "branch-reask", branchId: created.branchId },
      content: currentContent || q.data.content,
    });
    const agents = this.ctx.get("agents");
    const agent = agents && agents.get(created.childSessionId);
    if (!agent || typeof agent.followup !== "function") {
      return { ok: false, error: "agent 服务不可用，无法重新生成（分支已创建）", branchId: created.branchId, childSessionId: created.childSessionId };
    }
    try {
      agent.followup(reask);
    } catch (e) {
      return { ok: false, error: "重新生成注入失败: " + String(e && e.message ? e.message : e), branchId: created.branchId, childSessionId: created.childSessionId };
    }
    await this.vtd.branchSwitch(sessionId, created.branchId);
    let code = null;
    if (args.rollbackCode === true) {
      code = await this.restoreCodeToBreakpoint(q.time);
    }
    return { ok: true, branchId: created.branchId, childSessionId: created.childSessionId, anchorSeq: q.seq, code: code };
  }

  // ── VTD：分支视图（主线/分支 + 子会话流内联 + 节点 <N> 选择常显）──────────
  ...
  async branchView(args) {
    ...
    const parentEvents = Array.isArray(session.events) ? session.events : [];
    const parentMsgs = collapseWindowEvents(parentEvents, () => true).map(viewMessage).filter((m) => !m.reask);
    const branches = list.branches;
    const views = {};

    // 活动分支 + 祖先链（parentBranchId 上溯）
    const active = branches.find((b) => b.active) || null;
    const chain = new Map();
    let cur = active;
    while (cur) {
      chain.set(cur.branchId, true);
      cur = cur.parentBranchId ? branches.find((b) => b.branchId === cur.parentBranchId) : null;
    }
    // 每节点的子分支（anchorSeq 相同，按 createdAt 排序 → 创建顺序 1..n）
    const childrenBySeq = new Map();
    for (const b of branches) {
      const arr = childrenBySeq.get(b.anchorSeq) || [];
      arr.push(b);
      childrenBySeq.set(b.anchorSeq, arr);
    }
    for (const arr of childrenBySeq.values()) arr.sort((a, b) => a.createdAt - b.createdAt);

    /** 给消息行标注子分支选择（nodeKey = 该行在父会话中的分支锚点 seq）。 */
    const annotate = (msgs) => {
      for (const m of msgs) {
        if (m.nodeKey === null || m.nodeKey === undefined) { m.childBranches = []; m.branchIndex = null; m.selector = false; continue; }
        const children = childrenBySeq.get(m.nodeKey) || [];
        m.childBranches = children.map((c, i) => ({ branchId: c.branchId, index: i + 1 }));
        const chosen = children.find((c) => chain.has(c.branchId));
        m.branchIndex = chosen ? (children.indexOf(chosen) + 1) : (children.length >= 1 ? 1 : null);
        m.selector = children.length >= 1; // <N> 常显：节点有 ≥1 个子分支即显示
      }
      return msgs;
    };

    // 主线视图：整个当前 surface（分支锚点位置显示 <N> 选择）
    {
      const msgs = parentMsgs.slice(-500).map((m) => Object.assign({}, m, { nodeKey: m.seq }));
      views.trunk = { messages: annotate(msgs) };
    }

    const PREFIX_LIMIT = 60;
    const DIVERGENCE_LIMIT = 300;
    for (const branch of branches) {
      const anchorSeq = branch.anchorSeq;
      const forkBoundary = typeof branch.forkBoundary === "number" ? branch.forkBoundary : anchorSeq;
      // 子会话消息流（live 优先，否则冷读）
      const childEvents = branch.childSessionId ? await this.childSessionEvents(branch.childSessionId) : null;
      let divergence = [];
      if (childEvents) {
        divergence = collapseWindowEvents(childEvents, () => true)
          .map(viewMessage)
          .filter((m) => m.seq > forkBoundary && !m.reask)
          .slice(0, DIVERGENCE_LIMIT);
      }
      // 前缀：编辑类分支（首个分歧非 reask）替换锚点消息；重新生成/手动分叉保留锚点
      const firstIsReask = childEvents ? collapseWindowEvents(childEvents, () => true).some((m) => m.reask && m.seq > forkBoundary) : false;
      let prefix;
      if (divergence.length > 0 && !firstIsReask) {
        prefix = parentMsgs.filter((m) => m.seq < anchorSeq);
      } else {
        prefix = parentMsgs.filter((m) => m.seq <= anchorSeq);
      }
      if (prefix.length > PREFIX_LIMIT) prefix = prefix.slice(prefix.length - PREFIX_LIMIT);
      // 逐消息标注 nodeKey：前缀行用父 seq；分支首个分歧行映射到锚点 seq（<N> 显示处）
      const rows = prefix.map((m) => Object.assign({}, m, { nodeKey: m.seq }));
      divergence.forEach((m, i) => {
        rows.push(Object.assign({}, m, { nodeKey: i === 0 ? anchorSeq : null }));
      });
      views[branch.branchId] = { messages: annotate(rows) };
    }
    return { ok: true, branches: branches, views: views, activeBranchId: active ? active.branchId : null, trunk: branches.length === 0 };
  }

  // ── VTD：分支管理（真实子会话 = 后端对话，前端内联展示）──────────────────
  ...
  async branchCreate(args) {
    ...
    if (seq < 0) return { ok: false, error: "无法定位消息（会话未加载或消息不存在）" };
    // 手动分叉：在该消息处冷建真实子会话（空延续；之后可经分支输入框续写）
    return this.createChildBranch(session, seq, null, label || ("分叉 @" + seq));
  }

  async branchSwitch(args) {
    const sessionId = args && args.sessionId ? String(args.sessionId) : "";
    const branchId = args && args.branchId ? String(args.branchId) : "";
    if (branchId === "" || branchId === "trunk") return this.vtd.branchSwitch(sessionId, "trunk");
    const list = await this.vtd.branchList(sessionId);
    if (!list.ok) return list;
    const target = list.branches.find((b) => b.branchId === branchId);
    if (!target) return { ok: false, error: "分支不存在" };
    // 切到分支：确保子会话 agent 已运行（分支后续消息可续写）
    if (target.childSessionId) {
      const resumed = await this.resumeChildAgent(target.childSessionId);
      if (!resumed.ok) return { ok: false, error: resumed.error };
    }
    return this.vtd.branchSwitch(sessionId, branchId);
  }

  /** 分支输入：向活动分支（子会话）发送一条用户消息；trunk 则发给主会话。 */
  async branchSend(args) {
    const sessionId = args && args.sessionId ? String(args.sessionId) : "";
    const branchId = args && args.branchId ? String(args.branchId) : "";
    ...
    if (branchId !== "" && branchId !== "trunk") {
      const list = await this.vtd.branchList(sessionId);
      if (!list.ok) return list;
      const target = list.branches.find((b) => b.branchId === branchId);
      if (!target || !target.childSessionId) return { ok: false, error: "分支不存在或不是子会话分支" };
      const resumed = await this.resumeChildAgent(target.childSessionId);
      if (!resumed.ok) return { ok: false, error: resumed.error };
      targetId = target.childSessionId;
    }
    const agent = agents.get(targetId);
    if (!agent || typeof agent.followup !== "function") return { ok: false, error: "目标会话 agent 不可用" };
    try {
      agent.followup(msg);
    } catch (e) {
      return { ok: false, error: "发送失败: " + String(e && e.message ? e.message : e) };
    }
    return { ok: true };
  }
...
const METHOD_NAMES = [
  ...
  "branchList", "branchCreate", "branchSwitch", "branchDelete", "branchSend",
