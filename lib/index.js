...
class EssentialToolsService extends TypertRemoteService {
  ...
  /** 尽力判断会话是否 Full access 权限模式;无法确认时安全回退 false。 */
      ...
      if (sessions && typeof sessionId === "string" && sessionId !== "") {
        ...
        if (s && s.projections && typeof s.projections.faceOf === "function") {
          ...
          if (proj && typeof proj.getSnapshot === "function") {
            ...
            if (snap) {
              if (snap.currentValue === "danger-full-access") return true;
              if (snap.sandbox === "danger-full-access") return true;
              if (snap.preset === "danger-full-access") return true;
              if (snap.val && snap.val.sandbox === "danger-full-access") return true;
              if (snap.val && snap.val.preset === "danger-full-access") return true;
            }
          }
        }
      }
