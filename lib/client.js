      ...
      var card = function (p) {
        ...
        return React.createElement("div", { key: p.id, className: "dset-gp-card" },
          React.createElement("div", { className: "dset-gp-head" },
            ...
            React.createElement("span", { className: "dset-gp-badge" }, p.originKind === "cordis" ? "来自对话" : p.originKind === "github" ? "GitHub" : "URL")
          ),
          ...
          editingId[0] === p.id
            ? React.createElement("div", { className: "dset-gp-acts", style: { flexDirection: "column", alignItems: "stretch" } },
                React.createElement("input", { className: "dset-gp-input", value: editName[0], onChange: function (e) { setEditName(e.target.value); }, placeholder: "名称" }),
                React.createElement("textarea", { className: "dset-gp-input", rows: 2, value: editDesc[0], onChange: function (e) { setEditDesc(e.target.value); }, placeholder: "描述" }),
                React.createElement("div", null,
                  React.createElement("button", { className: "dset-btn-mini", disabled: busy[0], onClick: function () { saveMeta(p); } }, "保存"),
                  " ",
                  React.createElement("button", { className: "dset-btn-mini", onClick: function () { setEditingId(null); } }, "取消")
                )
              )
            : React.createElement("div", { className: "dset-gp-acts" },
                React.createElement("button", { className: "dset-btn-mini", disabled: busy[0], onClick: function () { if (!on) sessionEnable(p); else sessionDisable(p); } }, on ? "本会话停用" : "本会话启用"),
                React.createElement("button", { className: "dset-btn-mini", onClick: function () { setEditingId(p.id); setEditName(p.name); setEditDesc(p.description || ""); } }, "编辑"),
                React.createElement("button", { className: "dset-btn-mini", disabled: previewBusy[0][p.id] === true, onClick: function () { previewCode(p); } }, previews[0][p.id] ? "收起代码" : (previewBusy[0][p.id] ? "读取中…" : "预览代码")),
                React.createElement("button", { className: "dset-btn-mini dset-btn-danger", onClick: function () { removePlugin(p); } }, "删除")
              ),
          previews[0][p.id] ? React.createElement("div", { className: "dset-dbg-panel", style: { maxHeight: 280 } },
            previews[0][p.id].originRef ? React.createElement("div", { className: "dset-gp-store-sub" }, "来源: " + previews[0][p.id].originRef) : null,
            previews[0][p.id].warnings && previews[0][p.id].warnings.length
              ? React.createElement("div", { className: "dset-gp-note" },
                  "⚠ 可疑特征扫描: " + previews[0][p.id].warnings.map(function (w) { return w.half + ":" + w.label; }).join("; ") + "(提示项,请人工判断;扫描不是安全边界)")
              : React.createElement("div", { className: "dset-gp-store-sub" }, "未发现明显可疑特征(黑名单扫描非安全边界,请仍以信任为准)"),
            previews[0][p.id].host ? React.createElement("pre", { className: "dset-viewer-code" }, "── host 半区 ──" + NL + String(previews[0][p.id].host).slice(0, 6000) + (String(previews[0][p.id].host).length > 6000 ? "\n…(已截断)" : "")) : null,
            previews[0][p.id].client ? React.createElement("pre", { className: "dset-viewer-code" }, "── client 半区 ──" + NL + String(previews[0][p.id].client).slice(0, 6000) + (String(previews[0][p.id].client).length > 6000 ? "\n…(已截断)" : "")) : null
          ) : null
        );
      };
