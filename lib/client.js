...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function DetManagerSection(props) {
      ...
      // 总开关关闭时:列出被它一并停用的 DET 管控插件(可核对,而不是只能相信)。
      var pausedInfo = React.useState(null);
      var setPausedInfo = pausedInfo[1];
      React.useEffect(function () {
        if (masterOn) { setPausedInfo(null); return; }
        call("gpMasterState", {}).then(function (r) {
          setPausedInfo(r && r.ok ? r : null);
        }).catch(function () { setPausedInfo(null); });
      }, [masterOn]);
      ...
      var masterRow = React.createElement("div", {
        ...
        React.createElement("div", { className: "dset-switch-main" },
          ...
          React.createElement("div", { className: "dset-switch-sub" }, masterOn
            ...
            : "已关闭 · 完全原生:除本管理页与这个总开关外,DET 不再改动 DSH —— 它管控的插件(全局插件库里的常驻/会话插件)也已一并停用,重新打开会自动恢复")
        ),
      ...
      var rest = masterOn ? React.createElement("div", null,
        ...
        React.createElement("p", null, "总开关已关闭 —— DET 处于「完全原生」状态:界面注入、det_* 模型工具、系统提示注入、安全审计监听、浏览器桥与会话登记自检都已卸下,DSH 界面与产品原版一致,只剩本管理页。打开上面的总开关即可全部恢复(分项开关与插件启用的持久化设置原样保留)。"),
        // 被总开关一并停用的 DET 管控插件:关闭态下也要能核对「真的都停了」。
        (function () {
          var info = pausedInfo[0];
          if (info === null) return React.createElement("p", { className: "dset-sec-desc" }, "正在核对 DET 管控插件的状态…");
          // 管辖边界说清楚:不在全局插件库里的常驻插件(例如 dlt,它有自己的设置页开关)
          // 不归 DET 管,总开关既不列它也不碰它。
          var scopeNote = React.createElement("p", { className: "dset-sec-desc" },
            "管辖范围:只包括「全局插件管理」里那份插件库(dbs / topo 等)。不在库里的常驻插件(例如 dlt —— 它有自己的「DLT 管理器」总开关)不归 DET 管,本开关不会碰它们。");
          var unmanagedBlock = info.unmanagedCount
            ? React.createElement("div", { className: "dset-dbg-panel", style: { marginBottom: 8 } },
              React.createElement("div", { className: "dset-gp-store-sub" }, "库里声称常驻、但绑定不上已安装条目(" + info.unmanagedCount + " 个 · 没被停用):"),
              info.unmanaged.map(function (u) {
                return React.createElement("div", { key: u.id, className: "dset-dbg-row" },
                  React.createElement("span", { className: "dset-dbg-id" }, (u.name || u.id) + (u.moduleName ? " (" + u.moduleName + ")" : "")),
                  React.createElement("span", { className: "dset-dbg-tag", style: { color: "var(--dsw-alias-state-error-primary)" } }, u.reason || "未能停用")
                );
              })
            )
            : null;
          if (!info.pausedCount) {
            return React.createElement("div", null, scopeNote, unmanagedBlock,
              React.createElement("p", { className: "dset-sec-desc" }, "关闭时库里没有任何插件处于启用状态,无需停用。"));
          }
          return React.createElement("div", null,
            scopeNote,
            unmanagedBlock,
            React.createElement("div", { className: "dset-dbg-panel" },
              React.createElement("div", { className: "dset-gp-store-sub" },
                "已随总开关停用的 DET 管控插件(" + info.pausedCount + " 个" +
                (typeof info.stoppedCount === "number" ? " · 已确认停用 " + info.stoppedCount + " 个" : "") +
                (info.failedCount ? " · ⚠ 未能停用 " + info.failedCount + " 个" : "") +
                " · 重新打开总开关会自动恢复):"),
              info.paused.map(function (p) {
                return React.createElement("div", { key: p.id, className: "dset-dbg-row" },
                  React.createElement("span", { className: "dset-dbg-id" }, (p.name || p.id) + (p.moduleName ? " (" + p.moduleName + ")" : "")),
                  p.applied === false
                    ? React.createElement("span", { className: "dset-dbg-tag", style: { color: "var(--dsw-alias-state-error-primary)" } }, "⚠ 仍在运行:" + (p.reason || "未能停用") + " —— 可再关一次总开关重试")
                    : React.createElement("span", { className: "dset-dbg-tag" }, p.unverified ? "已停用(旧记录,未核对)" : (p.kind === "installed" ? "常驻已安装 · 已确认停止" : (p.permanent ? "常驻 · 已确认停止" : "会话实例已停"))),
                  p.sessionCount ? React.createElement("span", { className: "dset-dbg-tag" }, p.sessionCount + " 个会话记录保留") : null
                );
              }),
              // 宿主停了,浏览器里**已经在跑**的那半区不会自己消失:它的 client 包在入口
              // 被停用后就不再下发(见 dsh-client-modules),所以刷新页面即彻底干净。
              React.createElement("div", { className: "dset-sec-desc", style: { marginTop: 8 } },
                "宿主侧已按上面的核对结果停用。浏览器里已经在跑的插件界面(dbs 播放器、topo 圆点等)不会自己消失 —— 它们的 client 包在插件被停用后不再下发,刷新页面即彻底干净。"),
              React.createElement("button", {
                className: "dset-btn-mini",
                style: { marginTop: 6 },
                onClick: function () { try { window.location.reload(); } catch (e) { /* ignore */ } },
              }, "刷新页面(清掉已在跑的插件界面)")
            )
          );
        })()
    ...
    function GlobalPluginsSection(props) {
      ...
      var card = function (p) {
        ...
        // ── 指示状态一律按**实际**渲染(宿主已用 loader / 运行清单核对并回写记录)──
        // running:此刻是否真的在跑;hasRecord:是否只是留着一份启用记录。
        var running = !!(m && m.running === true && p.level !== "disabled");
        var hasRecord = !!(m && m.recordEnabled === true);
        var badgeText, badgeOn;
        if (p.permanent === true) {
          badgeOn = p.globallyEnabled !== false;
          badgeText = badgeOn ? "常驻插件已装载" : "常驻插件已禁用";
        } else if (running) {
          badgeOn = true; badgeText = "本会话运行中";
        } else if (hasRecord) {
          badgeOn = false;
          badgeText = m && m.runReason === "session-closed" ? "本会话未打开 · 打开后恢复" : "已启用记录 · 未运行";
        } else {
          badgeOn = false; badgeText = "本会话未启用";
        }
        var warn = [];
        if (p.stateMismatch === true) warn.push("记录与实际不一致,已按实际状态纠正为「" + (p.globallyEnabled === false ? "禁用" : "启用") + "」");
        if (p.entryFound === false) warn.push("loader 中找不到该常驻插件,装载状态未知");
        else if (p.fiberPhase === "failed") warn.push("装载失败(fiber=failed)");
        else if (p.fiberPhase && p.fiberPhase !== "active") warn.push("装载进行中(fiber=" + p.fiberPhase + ")");
        if (m && m.stale === true) warn.push("本会话有启用记录,但实例当前并未运行");
        return React.createElement("div", { key: p.id, className: "dset-gp-card" },
          React.createElement("div", { className: "dset-gp-head" },
            ...
            React.createElement("span", { className: "dset-gp-badge " + (badgeOn ? "dset-gp-badge-on" : "") }, badgeText),
          ...
          warn.length > 0 ? React.createElement("div", { className: "dset-gp-note" }, "⚠ " + warn.join(" · ")) : null,
          p.permanent
      ...
      return React.createElement("div", null,
        ...
        React.createElement("p", { className: "dset-sec-desc" }, "列表按【实际状态】显示:常驻插件以宿主 loader 为准(启用/禁用即真实装载状态),会话状态以运行清单为准(「运行中」= 此刻真的在跑;只有启用记录时会标注「未运行」)。记录与实际不一致时,DET 会自动按实际回写并在此提示。"),
