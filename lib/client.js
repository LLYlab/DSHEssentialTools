...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function mdaGroupByWorkspace(members) {
      ...
      var folders = [];
      Object.keys(byWs).sort().forEach(function (ws) { folders.push({ key: "ws:" + ws, name: ws, isAllWorkspace: false, members: byWs[ws] }); });
    ...
    function mdaBuildTree(mode, areas, sessions) {
      ...
      Object.keys(orphanWs).sort().forEach(function (ws) {
        var byName = null;
        for (var i = 0; i < modelNodes.length; i++) { if (modelNodes[i].name === ws) { byName = modelNodes[i]; break; } }
        if (byName) {
          var f = byName.folders.filter(function (x) { return !x.isAllWorkspace && x.name === ws; })[0];
          if (f) f.members = f.members.concat(orphanWs[ws]);
          else byName.folders.push({ key: byName.id + ":ws:" + ws, name: ws, isAllWorkspace: false, members: orphanWs[ws] });
        } else {
          modelNodes.push({ kind: "model", key: "__auto__" + ws, id: "__auto__" + ws, name: ws, workspace: ws, pluginSet: [], folders: [{ key: "__auto__" + ws + ":ws:" + ws, name: ws, isAllWorkspace: false, members: orphanWs[ws] }], synthetic: true });
        }
      });
    ...
    function MdaSidebar(props) {
      ...
      var rowCls = "dset-dbg-row";
      var rowBase = { alignItems: "center", gap: 6, height: 32, paddingRight: 8, fontSize: 13, color: "var(--dsw-alias-label-primary)", cursor: "pointer", minWidth: 0 };
      var grow = { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0 };
      var memberRow = function (s, pad) {
        return React.createElement("div", { key: s.id, className: rowCls, style: Object.assign({}, rowBase, { paddingLeft: pad }) },
          React.createElement("span", { style: { width: 14, textAlign: "center", color: "var(--dsw-alias-label-tertiary)" } }, "•"),
          React.createElement("span", { style: grow }, sessionTitle(s.id))
        );
      };
      var folderRow = function (f, modelKey, pad) {
        ...
        return React.createElement("div", { key: f.key },
          React.createElement("div", { className: rowCls, style: Object.assign({}, rowBase, { paddingLeft: pad }) },
            React.createElement("button", { className: "dset-vtd-ico", title: "折叠/展开", style: { fontSize: 10 }, onClick: function (e) { toggle(fk, e); } }, fo ? "▾" : "▸"),
            React.createElement("span", { style: { width: 14, color: "var(--dsw-alias-label-tertiary)", textAlign: "center" } }, f.isAllWorkspace ? "🗂" : "📁"),
            React.createElement("span", { style: Object.assign({}, grow, f.isAllWorkspace ? { color: "var(--dsw-alias-brand-primary)", fontWeight: 600 } : {}) }, f.name),
            React.createElement("span", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 11, flex: "none" } }, f.members.length)
          ),
          fo ? f.members.map(function (s) { return memberRow(s, pad + 16); }) : null
        );
      };
      var modelRow = function (m, pad) {
        ...
        return React.createElement("div", { key: m.id },
          React.createElement("div", { className: rowCls, style: Object.assign({}, rowBase, { paddingLeft: pad }) },
            React.createElement("button", { className: "dset-vtd-ico", title: "折叠/展开", style: { fontSize: 10 }, onClick: function (e) { toggle(mk, e); } }, mo ? "▾" : "▸"),
            React.createElement("span", { style: { width: 14, textAlign: "center" } }, "🤖"),
            React.createElement("span", { style: grow }, m.name),
            React.createElement("span", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 11, flex: "none", marginRight: 4 } }, mdaMemberCount(m)),
            m.synthetic ? null : React.createElement("button", { className: "dset-btn-mini", title: "组内新对话", disabled: disabled[0], onClick: function () { newConv(m.id, m.workspace || ""); } }, "➕")
          ),
          mo ? m.folders.map(function (f) { return folderRow(f, m.id, pad + 16); }) : null
        );
      };
      var wsRow = function (w, pad) {
        ...
        return React.createElement("div", { key: w.key },
          React.createElement("div", { className: rowCls, style: Object.assign({}, rowBase, { paddingLeft: pad }) },
            React.createElement("button", { className: "dset-vtd-ico", title: "折叠/展开", style: { fontSize: 10 }, onClick: function (e) { toggle(wk, e); } }, wo ? "▾" : "▸"),
            React.createElement("span", { style: { width: 14, textAlign: "center" } }, "📁"),
            React.createElement("span", { style: grow }, w.name),
            React.createElement("span", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 11, flex: "none", marginRight: 4 } }, w.models.length),
            React.createElement("button", { className: "dset-btn-mini", title: "建分组+新对话", disabled: disabled[0], onClick: function () { newArea(w.name === "(无工作区)" ? "" : w.name); } }, "➕")
          ),
          wo ? w.models.map(function (m) { return modelRow(m, pad + 16); }) : null
        );
      };
      ...
      var tree = mdaBuildTree(mode, st[0].areas, st[0].sessions);
      var modeOpts = [{ k: "native", l: "原生" }, { k: "workspace", l: "工作区组" }, { k: "model", l: "模型组" }];
      return React.createElement("div", { style: { padding: "2px 0 8px", minWidth: 0 } },
        React.createElement("div", { style: { display: "flex", gap: 4, padding: "6px 8px 8px" } },
        ...
        tree.kind === "native"
          ? (function () {
              ...
              return Object.keys(wsMap).sort().map(function (w) { return React.createElement("div", { key: w, className: rowCls, style: Object.assign({}, rowBase, { paddingLeft: 8 }) }, React.createElement("span", { style: { width: 14, textAlign: "center" } }, "📁"), React.createElement("span", { style: grow }, w), React.createElement("span", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 11, flex: "none" } }, wsMap[w].length)); });
            })()
          : tree.kind === "workspace"
    ...
    function MdaSidebarOverlay(props) {
      var call = props.call;
      var st = React.useState({ mode: "native", loaded: false });
      var dismissed = React.useState(false);
      var setDismissed = dismissed[1];
      var mode = st[0].mode;
      React.useEffect(function () { setDismissed(false); }, [mode]);
      // 自给:浮层自己 mount 时读一次模式(不依赖全局种子),据此决定是否显示。
      React.useEffect(function () {
        call("mdaGet", {}).then(function (r) {
          if (r && r.ok) st[1]({ mode: (r.mode || "native"), loaded: true });
          else st[1]({ mode: "native", loaded: true });
        }).catch(function () { st[1]({ mode: "native", loaded: true }); });
      }, []);
      var show = mode !== "native" && !dismissed[0];
      // 始终返回非空包裹层(指针穿透、透明),保证条目 active、挂载后必然执行上面的 mdaGet;
      // 仅当非原生且未关闭时才渲染右侧真正的“伪主侧边栏”内容。
      return React.createElement("div", { style: { position: "fixed", top: 0, left: 0, bottom: 0, width: 288, zIndex: 40, pointerEvents: show ? "auto" : "none" } },
        show
          ? React.createElement("div", { style: { display: "flex", flexDirection: "column", height: "100%", background: "var(--dsw-specific-sidebar-fill)", borderRight: "1px solid var(--dsw-alias-border-l1)", boxSizing: "border-box" } },
              // 顶部:仿原版侧边栏头部(左侧标签 + 右侧关闭),下方是紧凑的分组/会话列表
              React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", height: 36, padding: "0 8px 0 16px", color: "var(--dsw-alias-label-tertiary)", fontSize: 12, flex: "none" } },
                React.createElement("span", null, "工作区 · 会话"),
                React.createElement("button", { className: "dset-x", onClick: function () { setDismissed(true); } }, "×")
              ),
              React.createElement("div", { style: { flex: 1, overflow: "auto", minHeight: 0 } },
                React.createElement(MdaSidebar, {
                  call: call,
                  mode: mode,
                  onModeChange: function (m) { setMdaMode(m); st[1]({ mode: m, loaded: true }); }
                })
              )
            )
          : null
    ...
    function apply(ctx) {
      ...
      loadDsPrice(false);
      ctx.effect(function () {
        var timer = setInterval(function () { if (dsState.balanceLoaded) loadDsBalance(false); }, 20 * 1000);
      ...
      ctx.slots.inject("shell.overlay", function () {
        return ctx.slots.register({ name: "shell.overlay", id: "dsh-mda-overlay", order: 150 }, function (props) {
          return React.createElement(MdaSidebarOverlay, Object.assign({ call: call }, props));
        });
      });
      // 设置页:DET 管理器(功能开关 + 侧边栏数据自检 + VTD 调试)
