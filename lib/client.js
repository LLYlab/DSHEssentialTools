...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = "" +
      ...
      '.dset-tree-branch{opacity:.9;font-style:italic}' +
      ...
      '.dset-bview-composer{display:flex;flex-direction:column;gap:4px;padding:6px;border-top:1px solid var(--dsw-alias-border-l1);flex:none}' +
      '.dset-bview-composer-input{width:100%;box-sizing:border-box;min-height:44px;resize:vertical;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l1);border-radius:6px;padding:5px 6px;font:inherit;font-size:12px}' +
      ...
      '.dset-branch-pill{display:inline-flex;align-items:center;gap:4px;padding:2px 9px;border:1px solid var(--dsw-alias-border-l1);border-radius:999px;cursor:pointer;font-size:11.5px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);white-space:nowrap;position:relative}' +
      '.dset-branch-pill:hover{border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-primary)}' +
      '.dset-branch-pill-on{color:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-brand-primary)}' +
      '.dset-branch-pillmenu{position:absolute;top:calc(100% + 4px);right:0;min-width:190px;max-height:280px;overflow:auto;display:flex;flex-direction:column;gap:2px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l1);border-radius:8px;z-index:9998;box-shadow:0 8px 30px rgba(0,0,0,.35);padding:6px}';
    ...
    function BranchView(props) {
      ...
      var busy = React.useState(false);
      var setBusy = busy[1];
      var trunk = React.useState(false);
      var setTrunk = trunk[1];
      ...
      var refresh = function () {
        ...
        call("branchView", { sessionId: sessionId }).then(function (r) {
          ...
          if (r && r.ok) {
            ...
            setViews(r.views || {});
            setTrunk(r.trunk === true);
            setInfo(r);
            setSelected(function (prev) {
              if (r.trunk === true) return "trunk";
      ...
      var switchBranch = function (branchId) {
        ...
        call("branchSwitch", { sessionId: sessionId, branchId: branchId }).then(function (r) {
          setBusy(false);
          if (r && r.ok) { setSelected(branchId || "trunk"); refresh(); }
      ...
      // 分支输入：向当前选中分支（或主线）发送一条消息（branchSend → 子会话 agent 续答）
      var sendMsg = function () {
        var text = draft[0].trim();
        if (text === "" || busy[0]) return;
        setBusy(true);
        var target = selected[0] && selected[0] !== "trunk" ? selected[0] : "trunk";
        call("branchSend", { sessionId: sessionId, branchId: target, text: text }).then(function (r) {
          setBusy(false);
          if (r && r.ok) {
            setDraft("");
            setInfo({ ok: true, text: "✓ 已发送" });
            setTimeout(refresh, 1500);
          } else {
            setError("✗ " + ((r && r.error) || "发送失败"));
          }
        }).catch(function (e) {
          setBusy(false);
          setError("✗ " + String(e && e.message ? e.message : e));
      ...
      return React.createElement("div", { className: "dset-bview-root" },
        ...
        React.createElement("div", { className: "dset-bview-cols" },
          React.createElement("div", { className: "dset-bview-list" },
            branches[0].length === 0 && !trunk[0] && !loading[0]
              ? React.createElement("div", { className: "dset-empty" }, "暂无分支\n（在本视图内编辑/重试会创建分支）")
              : React.createElement(React.Fragment, null,
                  React.createElement("div", {
                    key: "trunk",
                    className: "dset-bview-item" + (selected[0] === "trunk" ? " dset-bview-item-on" : ""),
                    onClick: function () { switchBranch("trunk"); },
                  },
                    React.createElement("span", { className: "dset-bview-item-title" }, "主线" + (selected[0] === "trunk" ? "（当前）" : "")),
                    React.createElement("span", { className: "dset-bview-item-sub" }, "主会话 · 整个对话"),
                    React.createElement("button", {
                      className: "dset-branch-item-btn",
                      disabled: busy[0] || selected[0] === "trunk",
                      onClick: function (e) { e.stopPropagation(); switchBranch("trunk"); },
                    }, selected[0] === "trunk" ? "当前" : "切换")
                  ),
                  branches[0].map(function (b) {
                    return React.createElement("div", {
                      ...
                      React.createElement("span", { className: "dset-bview-item-sub" }, "锚点 " + b.anchorSeq + (b.childSessionId ? " · 子会话" : "") + " · " + (((views[0][b.branchId] || {}).messages) || []).length + " 条"),
                      React.createElement("button", {
                        className: "dset-branch-item-btn",
                        disabled: busy[0] || b.active,
          ...
          React.createElement("div", { ref: msgsRef, className: "dset-bview-msgs" },
            msgs.length === 0
              ? React.createElement("div", { className: "dset-empty" }, "该分支暂无消息")
              : msgs.map(renderMsg)
          ),
          React.createElement("div", { className: "dset-bview-composer" },
            React.createElement("textarea", {
              className: "dset-bview-composer-input",
              value: draft[0],
              placeholder: selected[0] && selected[0] !== "trunk" ? "在分支中发送…（后端子会话）" : "发送到主线…",
              onChange: function (e) { setDraft(e.target.value); },
              onKeyDown: function (e) { if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) { e.preventDefault(); sendMsg(); } },
            }),
            React.createElement("button", { className: "dset-branch-op", disabled: busy[0], onClick: sendMsg }, busy[0] ? "处理中…" : "发送")
          )
        ),
        dialog[0]
    ...
    // ── 我的消息操作（conversation.chat.user-actions，产品补丁新增槽）───────
    // 渲染在 user 气泡"复制"旁边：编辑 / 回退 + < N > 分支序号选择器。
    var bvCache = { sessionId: null, data: null, at: 0 };
    function fetchBranchViewCached(call, sessionId) {
      var now = Date.now();
      if (bvCache.sessionId === sessionId && bvCache.data !== null && now - bvCache.at < 5000) {
        return Promise.resolve(bvCache.data);
      }
      return call("branchView", { sessionId: sessionId }).then(function (r) {
    ...
    function UserActions(props) {
      ...
      var refreshNav = function () {
        ...
        fetchBranchViewCached(call, sessionId).then(function (r) {
          if (!r || !r.ok) return;
          var view = r.activeBranchId && r.views[r.activeBranchId];
          if (!view) return;
          var row = (view.messages || []).find(function (m) { return m.seq === seq; });
          if (row) setNav({ children: row.childBranches || [], index: row.branchIndex, selector: row.selector === true, activeBranchId: r.activeBranchId });
        }).catch(function () {});
      };
      ...
      var confirmChoice = function (choice) {
        ...
        if (d.kind === "edit") {
          ...
          if (text === "") { setBusy(false); setMsg({ ok: false, text: "内容不能为空" }); return; }
          extra = { newText: text };
        }
        runMsgAction(call, sessionId, messageId, d.kind, extra, choice === "confirm").then(function (r) {
          setBusy(false);
      ...
      var cycleBranch = function (dir) {
        var n = nav[0];
        if (!n || !n.selector || n.children.length < 2) return;
        var current = n.index || 1;
        var next = (current - 1 + dir + n.children.length) % n.children.length;
        setBusy(true);
        call("branchSwitch", { sessionId: sessionId, branchId: n.children[next].branchId }).then(function (r) {
          setBusy(false);
          bvCache.data = null;
          refreshNav();
      ...
      if (!messageId || !sessionId) return null;
      return React.createElement("span", { className: "dset-branch-wrap", style: { display: "inline-flex", alignItems: "center", gap: "2px" } },
        nav[0] && nav[0].selector
          ? React.createElement("span", { className: "dset-bview-nav" },
              React.createElement("button", { className: "dset-bview-nav-btn", disabled: busy[0], title: "上一个分支", onClick: function () { cycleBranch(-1); } }, "<"),
              React.createElement("span", { className: "dset-bview-nav-num" }, String(nav[0].index || 1)),
              React.createElement("button", { className: "dset-bview-nav-btn", disabled: busy[0], title: "下一个分支", onClick: function () { cycleBranch(1); } }, ">")
            )
          : null,
        React.createElement("button", { className: "dset-bview-act", disabled: busy[0], title: "编辑这条消息", onClick: function () { setEditMode(!editMode[0]); setEditText(editMode[0] ? "" : editText[0]); } }, "✎ 编辑"),
        React.createElement("button", { className: "dset-bview-act", disabled: busy[0], title: "回退这条消息", onClick: function () { setDlg({ kind: "rollback" }); } }, "↩ 回退"),
        editMode[0]
          ? React.createElement("span", { className: "dset-bview-editbox", style: { position: "absolute", right: 0, top: "calc(100% + 4px)", width: 260, zIndex: 9996, background: "var(--dsw-alias-bg-layer-1)", border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 8, padding: 6, boxShadow: "0 8px 30px rgba(0,0,0,.35)", display: "flex", flexDirection: "column", gap: 4 } },
              React.createElement("textarea", {
                className: "dset-bview-editarea",
                value: editText[0],
                placeholder: "新内容…",
              ...
              React.createElement("div", { className: "dset-branch-editbtns" },
                React.createElement("button", { className: "dset-branch-op", disabled: busy[0], onClick: function () { setDlg({ kind: "edit" }); } }, busy[0] ? "处理中…" : "保存"),
                React.createElement("button", { className: "dset-branch-op", disabled: busy[0], onClick: function () { setEditMode(false); } }, "取消")
              )
            )
          : null,
        msg[0]
          ? React.createElement("span", { className: "dset-branch-msg " + (msg[0].ok ? "dset-branch-msg-ok" : "dset-branch-msg-err"), style: { position: "absolute", right: 0, top: "calc(100% + 4px)", width: 260, zIndex: 9996, background: "var(--dsw-alias-bg-layer-1)", border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 8, padding: 6, whiteSpace: "pre-wrap", wordBreak: "break-word" } }, msg[0].text)
          : null,
        React.createElement(ConfirmDialog, { dialog: dlg[0], busy: busy[0], onChoice: confirmChoice })
      );
    }

    // ── 会话头：当前活跃分支指示器 + 切换（conversation.session.header.actions）──
    function BranchIndicator(props) {
      ...
      var active = branches[0].find(function (b) { return b.active; }) || null;
      var label = active ? (active.label || ("分支@" + active.anchorSeq)) : "主线";

      var doSwitch = function (branchId) {
        if (busy[0]) return;
        setBusy(true);
        call("branchSwitch", { sessionId: sessionId, branchId: branchId }).then(function () {
          setBusy(false);
          setOpen(false);
          bvCache.data = null;
          refresh();
      ...
      return React.createElement("span", {
        className: "dset-branch-pill" + (active ? " dset-branch-pill-on" : ""),
        title: "当前活跃分支（VTD）",
        onClick: function () { setOpen(!open[0]); },
      },
        "⑂ " + label + " ▾",
        open[0]
          ? React.createElement("span", { className: "dset-branch-pillmenu", onClick: function (e) { e.stopPropagation(); } },
              React.createElement("div", { className: "dset-branch-title" }, "切换到分支"),
              React.createElement("div", { className: "dset-branch-item" },
                React.createElement("span", { className: "dset-branch-item-label" }, "主线" + (!active ? "（当前）" : "")),
                React.createElement("button", {
                  className: "dset-branch-item-btn",
                  disabled: busy[0] || !active,
                  onClick: function () { doSwitch("trunk"); },
                }, !active ? "当前" : "切换")
              ),
              branches[0].length === 0
                ? React.createElement("div", { className: "dset-branch-msg" }, "暂无分支")
                : branches[0].map(function (b) {
                    return React.createElement("div", { key: b.branchId, className: "dset-branch-item" },
                      React.createElement("span", { className: "dset-branch-item-label" }, (b.label || ("分支@" + b.anchorSeq)) + (b.active ? "（当前）" : "")),
                      React.createElement("span", { className: "dset-branch-item-sub" }, "锚点 " + b.anchorSeq),
                      React.createElement("button", {
                        className: "dset-branch-item-btn",
                        disabled: busy[0] || b.active,
                        onClick: function () { doSwitch(b.branchId); },
                      }, b.active ? "当前" : "切换")
                    );
                  })
            )
    ...
    function Toolbar(props) {
      ...
      var countSessions = function (nodes) {
        var n = 0;
        for (var i = 0; i < nodes.length; i++) { n += 1; n += (nodes[i].branches || []).length; n += countSessions(nodes[i].children || []); }
        return n;
      };
      var renderTreeRow = function (node, depth) {
        ...
        var hasKids = kids.length > 0 || brs.length > 0;
        return React.createElement("div", { key: node.id },
          ...
          hasKids && isExpanded
            ? kids.map(function (k) { return renderTreeRow(k, depth + 1); })
                .concat(brs.map(function (b) {
                  return React.createElement("div", { key: "br-" + b.branchId },
                    React.createElement("div", {
                      className: "dset-tree-node dset-tree-branch",
                      style: { paddingLeft: String(10 + (depth + 1) * 16) + "px" },
                      title: "分支（后端子会话 " + b.childSessionId + "）· 锚点 " + b.anchorSeq,
                    },
                      React.createElement("span", { className: "dset-tree-caret" }, ""),
                      React.createElement("span", { className: "dset-tree-fork" }, "⑂"),
                      React.createElement("span", { className: "dset-tree-title" }, (b.label || "分支@" + b.anchorSeq) + (b.active ? "（当前）" : "")),
                      React.createElement("span", { className: "dset-tree-sub" }, "分支 · 锚点 " + b.anchorSeq)
                    )
                  );
                }))
            : null
        ...
        treeOpen[0]
          ? React.createElement("div", { className: "dset-tree-panel" },
              React.createElement("div", { className: "dset-head" },
                React.createElement("span", null, "会话树（fork 血缘 + 分支）"),
              ...
              React.createElement("div", { className: "dset-foot" }, "点击节点切换会话；带 ⑂ 的是 fork 子会话/分支")
            )
          : null;
    ...
    function apply(ctx) {
      ensureStyles();
      // 分支视图 = 默认主视图：产品聊天视图对 user 气泡没有操作插槽，
      // 因此强制预置会话视图为 dsh-branch（打开会话即分支感知界面，我的消息下有编辑）。
      // 同会话内仍可手动切回"聊天"；下次加载默认回到分支视图。
      try {
        if (typeof localStorage !== "undefined") {
          var rawView = localStorage.getItem("dsh.conversation.chat");
          var viewState = rawView ? JSON.parse(rawView) : null;
          localStorage.setItem("dsh.conversation.chat", JSON.stringify(Object.assign({ selection: null, draft: "", view: "dsh-branch", inspect: null }, viewState || {}, { view: "dsh-branch" })));
        }
      } catch (e) { /* 忽略 */ }
      ...
      // 我的消息操作（conversation.chat.user-actions —— 产品补丁新增的槽）
      ...
      ctx.slots.inject("conversation.session.header.actions", function () {
        return ctx.slots.register({
          ...
          id: "dsh-branch-indicator",
          order: 10,
        }, function (props) {
          return React.createElement(BranchIndicator, Object.assign({ call: call }, props));
        });
      });
