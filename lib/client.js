  ...
  factory: (require) => {
    ...
    function BranchAction(props) {
      ...
      var saveEdit = function () {
        if (opBusy[0] || !sessionId) return;
        var text = editText[0].trim();
        if (text === "") { setMsg({ ok: false, text: "✗ 内容不能为空" }); return; }
        setOpBusy(true);
        setMsg(null);
        call("msgEdit", { sessionId: sessionId, messageId: messageId, newText: text }).then(function (r) {
          setOpBusy(false);
          if (r && r.ok) { setEditMode(false); setMsg({ ok: true, text: "✓ 已编辑（旧内容已存为小版本）" }); }
      ...
      var doRollback = function () {
        if (opBusy[0] || !sessionId) return;
        setOpBusy(true);
        setMsg(null);
        call("msgRollback", { sessionId: sessionId, messageId: messageId }).then(function (r) {
          setOpBusy(false);
          if (r && r.ok) setMsg({ ok: true, text: "✓ 已回退到版本 " + r.restoredVersion + "（目标: " + r.target + "）" });
          else setMsg({ ok: false, text: "✗ " + ((r && r.error) || "回退失败") });
        }).catch(function (e) {
          setOpBusy(false);
          setMsg({ ok: false, text: "✗ " + String(e && e.message ? e.message : e) });
        });
      };
      var doRegenerate = function () {
        if (opBusy[0] || !sessionId) return;
        setOpBusy(true);
        setMsg(null);
        call("msgRegenerate", { sessionId: sessionId, messageId: messageId }).then(function (r) {
          setOpBusy(false);
          if (r && r.ok) setMsg({ ok: true, text: "✓ 已创建分支 " + r.branchId + " 并重新生成" });
          else setMsg({ ok: false, text: "✗ " + ((r && r.error) || "重新生成失败") });
        }).catch(function (e) {
          setOpBusy(false);
          setMsg({ ok: false, text: "✗ " + String(e && e.message ? e.message : e) });
        });
        ...
        open[0]
          ? React.createElement("div", { className: "dset-branch-menu", onClick: function (e) { e.stopPropagation(); } },
              ...
              React.createElement("div", { className: "dset-branch-ops" },
                React.createElement("button", { className: "dset-branch-op", disabled: opBusy[0], onClick: startEdit }, "✎ 编辑"),
                React.createElement("button", { className: "dset-branch-op", disabled: opBusy[0], onClick: doRollback }, "↩ 回退"),
                React.createElement("button", { className: "dset-branch-op", disabled: opBusy[0], onClick: doRegenerate }, "⟳ 重新生成")
              ),
              editMode[0]
    ...
    // ── "分支"视图（conversation.view 标签）：分支图 + 每分支消息流 ─────────
        ...
        React.createElement("div", { className: "dset-bview-cols" },
          ...
          React.createElement("div", { className: "dset-bview-msgs" },
            msgs.length === 0
              ...
              : msgs.map(function (m) {
                  return React.createElement("div", {
                    key: String(m.seq) + (m.messageId || ""),
                    className: "dset-bview-msg " + (m.role === "user" ? "dset-bview-msg-user" : "dset-bview-msg-assistant"),
                  },
                    React.createElement("div", { className: "dset-bview-msg-seq" }, "#" + m.seq + (m.role === "user" ? " · 用户" : " · 助手")),
                    m.text || "（无文本）"
                  );
                })
          )
